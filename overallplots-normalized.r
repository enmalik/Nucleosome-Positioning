# terminal arguments:
# Rscript overallplots-normalized.r #doesn't make any plots
# Rscript overallplots-normalized.r all #makes an overall scatterplot for all factors
# Rscript overallplots-normalized.r all 100 200 3 4 # makes a zoomed in plot of x from 100 to 200 and y from 3 to 4
# Rscript overallplots-normalized.r wgEncodeHaibTfbsGm12878Usf1Pcr2xPkRep1.broadPeak # example factor name, highlights the specific factor on the overall scatterplot
# Rscript overallplots-normalized.r wgEncodeHaibTfbsGm12878Usf1Pcr2xPkRep1.broadPeak 100 200 3 4 # zoomed in version, if factor is on window, will be highlighted
...
scatterPlotData <- read.table("stats/scatter-plot-data-normalized.txt", as.is=TRUE, col.names=c("scatterDistance", "scatterIntensity", "tfColour", "scatterLabel", "scatterNumberLabel", "scatterNumberNameLabel", "scatterPointColour", "scatterName"));
...
addZoomPlot <- function(limX1, limX2, limY1, limY2) {
  ...
  fileName <- paste("stats/OverallScatterPlot-Numbered",highlightName, "-",limX1,"-",limX2,"-",limY1,"-",limY2,"-normalized.png",sep="")
  print(fileName)
...
if (is.na(args[2]) == TRUE & (highlightFactor == "all" | highlightFactor != "")) {
  # print("here")
  png(paste("stats/OverallScatterPlot-Numbered-Labels", highlightName, "-normalized.png", sep=""), width = 4000, height = 3500, res = 80);
  ...
  png(paste("stats/OverallScatterPlot-non-Numbered", highlightName, "-normalized.png", sep=""), width = 2000, height = 1500, res = 80);
